    ...
    if(xmin == Double.MaxValue) {
            ...
            cfor(0)(_<rows,_+1) { row =>
              ...
              if(l.isEmpty) {
                ...
                cfor(0)(_ < l.length, _ + 1) { i =>
                  ...
                  val d = math.sqrt(dlat*dlat+dlong*dlong)
                  val w = 1/(d*d)
                  s += t * w
                  ws += w
                  c += 1
      ...
      val palette = 
        List(0x4698D3FF, 0x39C6F0FF,
          ...
          0xF16022FF, 0xEE2C24FF
)

      val rOp = 
        r.mapIfSet { z => 
          val interval = z % (1 * 60)
          palette(interval % palette.length)
        }
